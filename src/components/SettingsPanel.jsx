...
function Section({ title, children }) {
  return (
    <div className="space-y-2">
      <p className="text-[10px] font-bold uppercase tracking-[0.12em] font-mono" style={{ color: 'var(--accent)' }}>{title}</p>
...
export default function SettingsPanel() {
  ...
  useEffect(() => {
    const h = (e) => { if (e.key === 'Escape') closeSettings(); };
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      <div className="flex-1 modal-backdrop bg-black/50" onClick={closeSettings}/>
      <div className="w-80 border-l border-app-border flex flex-col h-full shadow-2xl fade-in overflow-hidden bg-app-bg">
        <div className="flex items-center justify-between px-5 py-4 border-b border-app-border">
          <h2 className="text-xs font-bold uppercase tracking-[0.12em] font-mono" style={{ color: 'var(--accent)' }}>AYARLAR</h2>
          <button onClick={closeSettings} className="w-7 h-7 flex items-center justify-center text-[#5c665a] hover:text-white hover:bg-app-surface transition-colors text-xs">✕</button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6">
              ...
              <div className="flex overflow-hidden border border-app-border">
                {['dark', 'light'].map((t) => (
                  <button
                    ...
                    style={settings.theme === t ? { background: 'var(--accent)' } : {}}
                  >
                    {t === 'dark' ? 'KOYU' : 'AÇIK'}
                  </button>
                ))}
          ...
          <Section title="Ses">
            ...
            >
              <input
                ...
                onChange={(e) => updateSettings({ globalVolume: parseFloat(e.target.value) })}
          ...
          <Section title="Veri">
            ...
            <div className="p-3 bg-app-surface border border-app-border flex items-center justify-between">
              ...
              <p className="text-xs font-mono" style={{ color: 'var(--accent)' }}>{sounds.length} SES · {categories.length} KAT</p>
            </div>
          ...
          <Section title="Hakkında">
            <div className="bracket-4 p-4 bg-app-surface text-center space-y-2">
              <div className="w-10 h-10 flex items-center justify-center mx-auto" style={{ background: 'var(--accent)' }}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="#0b0d0b">
                  <path d="M12 3v10.55c-.59-.34-1.27-.55-2-.55-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4V7h4V3h-6z"/>
                </svg>
              </div>
              <p className="text-sm font-bold font-mono tracking-widest text-white">SOUNDBOARD</p>
              <p className="text-[10px] font-mono text-[#5c665a]">v2.0.0 · ELECTRON + REACT</p>
              <p className="text-[10px] font-mono text-[#3c4238]">VB-AUDIO · OBS · TWITCH</p>
            </div>
          </Section>
        </div>
      </div>
